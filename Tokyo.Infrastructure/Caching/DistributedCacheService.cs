...
namespace Tokyo.Infrastructure.Caching
{
    public class DistributedCacheService : ICacheService
    {
        ...
        public DistributedCacheService(IDistributedCache cache, ISerializerService serializer, Serilog.ILogger logger) =>
        ...
        public async Task RefreshAsync(string key, CancellationToken token = default)
        {
            try
            {
                ...
                _logger.Debug(string.Format("Cache Refreshed : {0}", key));
            }
            catch
            {
            }
        ...
        private void Set(string key, byte[] value, TimeSpan? slidingExpiration = null)
        {
            try
            {
                ...
                _logger.Debug($"Added to Cache : {key}");
            }
            catch
            {
            }
